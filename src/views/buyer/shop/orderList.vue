...
<script>
  ...
  export default {
    ...
    deactivated() {
      if (this.$route.path == '/orderDetail' || this.$route.path == '/logisticsInformation' || this.$route.path ==
        '/addComment') {
    ...
    methods: {
      ...
      deleteOrder(item) {
        MessageBox.confirm('是否删除该订单？').then(() => {
          axios.post(api.order_delete + '?token=' + localStorage.getItem('token'), {
            omid: item.omid
          }).then(res => {
            if (res.data.status == 200) {
              this.reload();
      ...
      orderConfirm(items) {
        MessageBox.confirm('是否确认该订单的收货？').then(() => {
          axios.post(api.order_confirm + '?token=' + localStorage.getItem('token'), {
            omid: items.omid
          }).then(res => {
            if (res.data.status == 200) {
              this.reload();
