<template>
  <div>
    <div class="m-one-group" :class="items.gpstatus == 20?'m-cancel':''" @click="changeRoute('/magicProductDetail',items)" v-for="(items,index) in magic_list">
      ...
      <div class="m-user-box">
        ...
        <p class="m-group-btn">{{items.record}} </p>
      </div>
    </div>
  </div>
    ...
    methods:{
          ...
          if(res.data.status == 200){
            ...
            if(res.data.data.length > 0) {
              if(this.page_info.page_num > 1) {
                this.magic_list = this.magic_list.concat(res.data.data);
              }else{
                this.magic_list = res.data.data;
              }
      ...
      touchMove(e) {
        ...
        if (scrollTop + ClientHeight  >= scrollHeight -10) {
          if(this.isScroll){
            this.isScroll = false;
            if(this.magic_list.length == this.total_count){
