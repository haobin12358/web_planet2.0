<template>
  <div class="m-activity-product-detail">
    ...
    <!--选择sku-->
    <sku v-if="show_sku" :now_select="select_value" :guess="true" :now_num="canums" :product="product" @changeModal="changeModal" @sureClick="sureClick" :activity="true"></sku>
    ...
    <div class="m-text-row m-sku-row" @click="changeModal('show_sku', true)">
      ...
      <div class="m-text-description">
        ...
        <template v-else>
          <span v-for="item in product.prattribute">{{item}} </span>
        </template>
      </div>
